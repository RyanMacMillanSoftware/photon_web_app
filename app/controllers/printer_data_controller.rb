class PrinterDataController < ApplicationController
    ...
    to_time = DateTime.now.change(hour: hour, min: params[:'minute'].to_i, sec: 1)
   		@printer_data = PrinterDatum.new(name: params[:printer_datum][:'name'], project: params[:printer_datum][:'project'], printer: PrinterStatus.find(params[:printer_id]), phonenumber: params[:printer_datum][:'phonenumber'], volume: params[:printer_datum][:'volume'], notes: params[:printer_datum][:'notes'], from_time: DateTime.now, to_time: to_time)
    	if @printer_data.save
        #update printer status
        printer = PrinterStatus.find(params[:printer_id])
